...
Etapa zero da cadeia `ambion_constituir` → `ambion_plain` → `ambion_spec` → `ambion_task` → `ambion_execute` → `ambion_review`. Diferente das demais, não é por feature — roda uma vez para o projeto inteiro e é revisada sempre que um padrão mudar, não a cada atividade nova.
...
4. **Qualidade de código**: regras de lint/formatação, convenções de nomenclatura, limites de tamanho de função/arquivo, política de comentários, quem revisa/aprova. Proponha a partir das ferramentas já configuradas no repo antes de perguntar.
...
6. **Qualidade de manutenção**: limites de complexidade/duplicação aceitáveis, quando um ADR é obrigatório, documentação mínima exigida por tipo de mudança.

...
interface DBConfig {
  name: string;       // chave interna (ex: "analytics")
  label: string;      // nome amigável exibido ao Claude
  host: string;
  port: number;
  database: string;
  user: string;
  ...
  ssl: boolean;
  poolMax: number;
  readonly: boolean;  // bloqueia execute/transaction se true
}
...
function loadConfigs(): DBConfig[] {
  ...
  if (names.length === 0) {
    console.error(
      "⚠️  PG_SERVERS não definido ou vazio. Defina no .env: PG_SERVERS=principal,analytics"
    );
    process.exit(1);
  }
  ...
  for (const name of names) {
    ...
    if (!host)     missing.push(`PG_${name}_HOST`);
    if (!database) missing.push(`PG_${name}_DATABASE`);
    if (!user)     missing.push(`PG_${name}_USER`);
    if (!password) missing.push(`PG_${name}_PASSWORD`);

    if (missing.length > 0) {
      console.error(
        `❌ Servidor "${name}" ignorado — variáveis faltando: ${missing.join(", ")}`
      );
      continue;
    }
  ...
  console.error(
    `✅ Servidores carregados: ${configs.map((c) => `${c.name} (${c.label})`).join(" | ")}`
  );
...
function getPool(dbName: string): pg.Pool {
  ...
  if (!pools.has(key)) {
    ...
    if (!cfg) {
      ...
      throw new Error(
        `Servidor "${dbName}" não encontrado. Disponíveis: ${available}`
      );
    }
...
function getConfig(dbName: string): DBConfig {
...
function serverInfo(cfg: DBConfig): string {
...
server.tool(
  "list_databases",
  "Lista todos os servidores PostgreSQL configurados e disponíveis.",
  {},
  async () => {
...
server.tool(
  "query",
  "Executa SELECT no PostgreSQL. Use list_databases para ver os servidores disponíveis.",
  {
...
server.tool(
  "execute",
  "Executa INSERT, UPDATE, DELETE ou DDL no PostgreSQL. Requer confirm: true. Bloqueado em servidores readonly.",
  {
...
server.tool(
  "list_tables",
  "Lista tabelas e views de um schema, com tamanho em disco.",
  {
...
server.tool(
  "describe_table",
  "Descreve colunas, tipos, PKs, FKs e índices de uma tabela.",
  {
...
server.tool(
  "transaction",
  "Executa múltiplos SQL em transação ACID. ROLLBACK automático em qualquer erro. Bloqueado em servidores readonly.",
  {
